...
from workflow import orchestrate, SearchQueries
...
async def test_orchestrate_standard_simple_query(client: OAI):
    ...
    messages = [
        Message(role="user", content="What is the current price of Bitcoin?")
    ]
...
async def test_orchestrate_standard_complex_query(client: OAI):
    ...
    messages = [
        Message(role="user", content="Compare the pros and cons of React vs Vue for building a large-scale enterprise application")
    ]
...
async def test_orchestrate_deep_research_with_context(client: OAI):
    ...
    messages = [
        Message(
            role="user", 
            content="What are the health benefits and risks of intermittent fasting?"
        )
    ]
...
"""
    ...
    print(f"User: {messages[0].content}")
    print(f"Previous context provided: Yes")
    print(f"Queries generated: {len(queries.queries)}")
    for i, q in enumerate(queries.queries, 1):
        print(f"  {i}. {q}")
    print(f"Tokens: {output.usage.total_tokens}")
