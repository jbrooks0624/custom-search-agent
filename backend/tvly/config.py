...
class TavilyConfig(BaseModel):
    ...
    time_range: TimeRange | None = Field(
        default=None,
        description="Filter results by time range: 'day', 'week', 'month', 'year'"
    )
    include_domains: list[str] = Field(
        default_factory=list,
        description="Domains to specifically include (max 300)"
    )
    exclude_domains: list[str] = Field(
        default_factory=list,
        description="Domains to specifically exclude (max 150)"
    )
    timeout: float = Field(
        default=60.0,
        gt=0,
