...
_SOCIAL_PATTERNS = [
    ...
    re.compile(r'^\s*\[?(Facebook|Twitter|LinkedIn|Instagram|YouTube|Pinterest|Reddit|WhatsApp|Telegram|Email)\]?\s*$', re.MULTILINE | re.IGNORECASE),
    re.compile(r'^\s*Share on (Facebook|Twitter|LinkedIn|WhatsApp).*$', re.MULTILINE | re.IGNORECASE),
...
_RELATED_PATTERNS = [
    re.compile(r'^#+\s*(Related|Recommended|Popular|Trending|More|You (might|may) also (like|enjoy)|See also|Further reading).*$', re.MULTILINE | re.IGNORECASE),
    re.compile(r'^\s*(Related|Recommended) (articles?|posts?|stories|content|links?).*$', re.MULTILINE | re.IGNORECASE),
